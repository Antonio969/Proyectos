...
Polinomio creaP(int deg) // 4
{
    ...
    P.coef = (Matriz*)malloc((deg+1)*sizeof(Matriz)); // 0 1 2 3
...
{
    Polinomio prod={deg:P.deg+Q.deg}; // 4 + 3 = 7-1
    int i,j,a,b;
    ...
    for(i=0;i<=P.deg;i++){
        for(j=0;j<=Q.deg;j++){
            prod.coef[i+j]=creaM(n); // Crear coeficiente de tamaño n (2)

            for(b=0; b<n; b++){
                for(a=0; a<n; a++){
                    prod.coef[i+j].ent[a][b].re.num = 1;
                    prod.coef[i+j].ent[a][b].re.den = 1;
                    prod.coef[i+j].ent[a][b].im.num = 1;
                    prod.coef[i+j].ent[a][b].im.den = 1;
                }
            }
...
Matriz evP(Matriz A, Polinomio P, int n)
{
    ...
    if(P.deg!=0){
    	for(i=P.deg;i>0;i--){          
       	 O=sumaM(productoM(A,A), P.coef[i]);
    	}
	}
